...
when "debian", "ubuntu"
  ...
  %w{git-core build-essential pkg-config libtool libncurses-dev zlib1g-dev automake libssh-dev cmake vim}.each do |pkg|
    package pkg do
      action :install
    end
  end
...
when "redhat", "centos", "fedora","amazon","scientific"
  ...
  remote_file "#{Chef::Config[:file_cache_path]}/v1.8.4.1.tar.gz" do
    ...
    owner 'root'
    group 'root'
    notifies :run, 'bash[build git]', :immediately
  end
